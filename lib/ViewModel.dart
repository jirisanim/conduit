...
class CalculatorControl {
  ...
  void resultOnPressed(BuildContext context) {
    ...
    if (display.length >= 3 && display.startsWith('.0', display.length - 2)) {
      display = display.substring(0, display.length - 2); // 정수일 경우 소수점 이하 삭제
    }
  ...
  void functionOnPressed(BuildContext context, String st) {
    ...
    if (st == 'C') // clear input
    {
      ...
      var imsi =
          double.parse(context.read<DisplayNumValue>().displayValue) / 100;
  ...
  void numberOnPressed(BuildContext context, String st) // 숫자키 입력 이벤트
  {
    bool inputAdd = true;
    ...
    if (st == '.') // 소숫점이 눌려졌을 경우
    {
      if (makeNumber.isEmpty == true) {
        ...
        if (pointExist == true) inputAdd = false; // 소숫점이 없을 경우
      }
